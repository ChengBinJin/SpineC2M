    ...
    def _make_folders(self):
        if self.flags.is_train:  # train stage
            if self.flags.load_model is None:
                ...
                self.model_out_dir = "{}/VAE_model/{}".format(self.flags.dataset, cur_time)
            ...
            self.sample_out_dir = "{}/VAE_sample/{}".format(self.flags.dataset, cur_time)
            if not os.path.isdir(self.sample_out_dir):
                os.makedirs(self.sample_out_dir)

            self.log_out_dir = "{}/VAE_logs/{}".format(self.flags.dataset, cur_time)
            self.train_writer = tf.summary.FileWriter("{}/VAE_logs/{}".format(self.flags.dataset, cur_time),
                                                      graph_def=self.sess.graph_def)

        elif not self.flags.is_train:  # test stage
            self.model_out_dir = "{}/VAE_model/{}".format(self.flags.dataset, self.flags.load_model,)
            self.test_out_dir = "{}/VAE_test/{}".format(self.flags.dataset, self.flags.load_model)
            self.log_out_dir = "{}/VAE_logs/{}".format(self.flags.dataset, self.flags.load_model)
